...
def get_results(df):
    """Get the top 5 musical genres that are registered the most in the playlists

    Args:
        df (Object): Get the created pandas dataframe

    Returns:
        Object: Returns a new dataframe with the 5 genres that appear the most in the playlists
    """
    result_df = df.groupby(['genre'])['genre'].count().sort_values(ascending=False).iloc[0:5]
...
if __name__ == '__main__':
    ...
    print('\nGetting results of the Hypothesis . . .')
    print('\nTop 5 genres:')
    results = get_results(df_clean)
    print(results)
